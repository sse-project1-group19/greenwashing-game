...
export const updatePerceptionOnTick = (state: GameState) => {
    ...
    var turn = state.turn;
    var degradationFactor = map(turn, 0, 1000, 0.001, 0.55); // Example degradation factor
    var perceptionDegradation = Math.max(0, perceptionMax * degradationFactor);
    console.log(`Turn: ${turn}, Current perception: ${state.perceptionCurrent}, Perception Degradation: ${perceptionDegradation.toFixed(2)}`);
